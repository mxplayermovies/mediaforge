...
export default function Header() {
  ...
  // --- GOOGLE TRANSLATE ROBUST INITIALIZATION ---
  ...
  const initGoogleTranslate = useCallback(() => {
    if ((window as any).google && (window as any).google.translate) {
      const config = {
        pageLanguage: 'en',
        // includedLanguages: 'ar,zh-CN,en,es,fr,hi,ja,ko,pt,ru', // REMOVED to support ALL languages
        layout: (window as any).google.translate.TranslateElement.InlineLayout.VERTICAL, // CHANGED TO VERTICAL (Scroll down format)
        autoDisplay: false,
      };

      // Desktop Initialization
      const desktopId = 'google_translate_element_desktop';
      const desktopEl = document.getElementById(desktopId);
      if (desktopEl) {
          desktopEl.innerHTML = ''; // Clean up previous instance
          try {
            new (window as any).google.translate.TranslateElement(config, desktopId);
          } catch (e) {
            console.error('Google Translate Desktop Init Error:', e);
          }
      }

      // Mobile Initialization
      const mobileId = 'google_translate_element_mobile';
      const mobileEl = document.getElementById(mobileId);
      if (mobileEl) {
          mobileEl.innerHTML = ''; // Clean up previous instance
          try {
            new (window as any).google.translate.TranslateElement(config, mobileId);
          } catch (e) {
            console.error('Google Translate Mobile Init Error:', e);
          }
      }
    }
  }, []);
  ...
  useEffect(() => {
    // 1. Assign the global callback required by the Google Script
    (window as any).googleTranslateElementInit = initGoogleTranslate;

    // 2. Check if the script is already loaded and initialized
    if ((window as any).google && (window as any).google.translate) {
      initGoogleTranslate();
    }
  }, [initGoogleTranslate]);

  // --- LANGUAGE DETECTION FOR VOICE MANAGER ---
  useEffect(() => {
    // Observer to watch for language changes in the <html> tag
    // Google Translate modifies <html lang="en" class="translated-ltr">
  ...
  const headerBgClass = isScrolled || mobileMenuOpen || router.pathname.includes('/studio')
    ? 'bg-white/90 dark:bg-[#0f172a]/90 backdrop-blur-md shadow-sm border-b border-slate-200 dark:border-white/5 py-2' 
    : 'bg-transparent py-4';
    
  const textClass = isScrolled || mobileMenuOpen || router.pathname.includes('/studio') 
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14 md:h-16">
            ...
            >
              ...
              <div className="flex flex-col">
                <span className={`font-bold text-lg md:text-xl tracking-tight leading-none ${textClass}`}>
                  Media<span className="text-blue-500">Forge</span>
                </span>
                <span className="text-[10px] text-slate-500 dark:text-slate-400 font-medium tracking-widest uppercase leading-none mt-0.5">
                  Studio
            ...
            <nav className="hidden md:flex items-center gap-6 lg:gap-8">
              ...
              {/* Google Translate Desktop Container */}
              <div className="flex items-center bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-lg sm:rounded-xl px-3 py-1.5 gap-2 transition-colors text-slate-600 dark:text-slate-300">
                  <Languages size={14} className="text-blue-600 dark:text-blue-400 shrink-0" />
                  <div id="google_translate_element_desktop" className="min-w-[100px] min-h-[24px] flex items-center justify-center"></div>
              </div>

              <Link href="/features" className={`text-sm font-medium transition-colors hover:text-blue-500 ${isActive('/features') ? 'text-blue-500' : 'text-slate-600 dark:text-slate-300'}`}>Features</Link>
              <Link href="/how-it-works" className={`text-sm font-medium transition-colors hover:text-blue-500 ${isActive('/how-it-works') ? 'text-blue-500' : 'text-slate-600 dark:text-slate-300'}`}>How it Works</Link>
              ...
              <Link href="/studio" className="px-5 py-2.5 rounded-full bg-blue-600 text-white text-sm font-bold hover:bg-blue-500 transition-all shadow-lg shadow-blue-500/30">
                Launch Studio
              </Link>
            </nav>
      ...
      {/* Mobile Menu Overlay */}
      <div className={`fixed inset-0 bg-white/95 dark:bg-[#0f172a]/98 backdrop-blur-xl z-[90] transition-all duration-300 md:hidden flex flex-col pt-24 px-6 ${mobileMenuOpen ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-full pointer-events-none'}`}>
        <nav className="flex flex-col gap-6 items-center w-full">
          
          {/* Google Translate Mobile Container */}
          <div className="flex items-center bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-3 gap-3 w-full max-w-xs justify-center text-slate-600 dark:text-slate-300">
                <Languages size={18} className="text-blue-600 dark:text-blue-400 shrink-0" />
                <div id="google_translate_element_mobile" className="min-w-[120px] min-h-[24px] flex items-center justify-center"></div>
          </div>

          <Link 
            href="/features" 
            className="text-2xl font-bold text-slate-800 dark:text-slate-100 hover:text-blue-500 py-2"
            onClick={() => setMobileMenuOpen(false)}
          >
            Features
          </Link>
          ...
          <div className="w-20 h-1 bg-slate-200 dark:bg-slate-800 rounded-full my-4"></div>
